...
#define WIFI_TXINJECT_ENABLE_MIN 0
#define WIFI_TXINJECT_ENABLE_MAX 1

static int cmd_wifi_txinjection(const struct shell *sh, size_t argc, char *argv[]) 
{
	int ret;
	struct net_if *iface;
	long enable = 0;
	...
	if (!parse_number(sh, &enable, argv[1],
			  WIFI_TXINJECT_ENABLE_MIN,
			  WIFI_TXINJECT_ENABLE_MAX)) {
		return -EINVAL;
	}
	...
	if (ret != 0) {
		shell_fprintf(sh, SHELL_ERROR, "iface %p txinjection get mode failed: %d", iface, ret);
        } else {
		shell_fprintf(sh, SHELL_ERROR, "iface %p txinjection get mode: %d", iface, params.txinjection_mode);
	}

	ret = net_eth_txinjection_mode(iface, enable);
	
	if (ret != 0) {
		shell_fprintf(sh, SHELL_ERROR, "iface %p txinjection mode failed: %d", iface, ret);
	}
...
static int cmd_wifi_promisc(const struct shell *sh, size_t argc, char *argv[])
{
       ...
       ret = net_eth_promisc_mode(iface, enable);
       if (ret != 0) {
               shell_fprintf(sh, SHELL_ERROR, "iface %p promisc mode failed: %d", iface, ret);
       }
...
static int cmd_wifi_mode(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (do_mode_oper) {
		...
		shell_fprintf(sh, SHELL_ERROR, "mode_info operation is %d\n", mode_info.oper);
...
SHELL_STATIC_SUBCMD_SET_CREATE(wifi_commands,
	...
	SHELL_CMD(raw_packet, NULL, "send a test raw packet\n"
		...
		"wifi raw_packet\n",
		cmd_wifi_rawpacket),
	SHELL_CMD(txinject_set, NULL, "set txinjection mode\n"
		"This command may be used to enable or disable txinjection mode\n"
		"Usage: \n"
		"wifi txinject_set 1 or 0\n",
		cmd_wifi_txinjection),
	SHELL_CMD(promisc_set, NULL, "set promisc mode\n"
		"This command may be used to enable or disable promiscuous mode\n"
		"Usage: \n"
		"wifi promisc_set 1 or 0\n",
		cmd_wifi_promisc),	
